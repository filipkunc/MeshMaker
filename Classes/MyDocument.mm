...
- (BOOL)aiSceneNeedsTriangulation:(const aiScene *)scene
{
    for (uint i = 0; i < scene->mNumMeshes; i++)
    {
        ...
        {
            if (mesh->mFaces[j].mNumIndices > 4)
            {
                return YES;
            }
        }
...
{
    if ([typeName isEqualToString:@"model3D"] || [typeName isEqualToString:@"folder3D"])
    ...
    const aiScene *scene = importer.ReadFile([filePath UTF8String], 0);
    if (!scene)
    {
        NSLog(@"Assimp error: %s", importer.GetErrorString());
        return NO;
    }
    ...
    if (scene->HasMeshes())
    {
        ...
        for (uint i = 0; i < scene->mNumMeshes; i++)
        {
            ...
            {
                TriQuad triangle;
                triangle.isQuad = mesh->mFaces[j].mNumIndices == 4;
                for (uint k = 0; k < mesh->mFaces[j].mNumIndices; k++)
                {
                    triangle.vertexIndices[k] = mesh->mFaces[j].mIndices[k];
                    triangle.texCoordIndices[k] = mesh->mFaces[j].mIndices[k];
                }
...
- (BOOL)writeToURL:(NSURL *)url ofType:(NSString *)typeName error:(NSError *__autoreleasing *)outError
{
    ...
    scene->mNumMeshes = items.count;
    scene->mMeshes = new aiMesh *[scene->mNumMeshes];
    scene->mRootNode = new aiNode();
    scene->mRootNode->mNumChildren = items.count;
    ...
    for (Item *item in items)
    {
        ...
        itemMesh->toIndexRepresentation(vertices, texCoords, triangles);
        ...
        for (uint j = 0; j < mesh->mNumVertices; j++)
            mesh->mVertices[j] = aiVector3D(vertices[j].x, vertices[j].y, vertices[j].z);
        ...
        for (uint j = 0; j < mesh->mNumFaces; j++)
        {
            ...
            TriQuad &triQuad = triangles[j];
            FlipTriangle(triQuad);
            
            face.mNumIndices = triQuad.isQuad ? 4 : 3;
            face.mIndices = new uint[face.mNumIndices];
            for (uint k = 0; k < face.mNumIndices; k++)
                face.mIndices[k] = triQuad.vertexIndices[k];
        }
    ...
    if (exporter.Export(scene, [[url pathExtension] UTF8String], [[url path] UTF8String]) == AI_SUCCESS)
    {
        delete scene;
        return YES;
    }
